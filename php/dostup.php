...
if (isset($_GET['save'])) {
    ...
    $db->query("DELETE FROM dle_cat_dostup WHERE id_category = {$id_cat} AND category = $category $dproject");

    foreach ($rol as $key => $value) {
        if ($value['user'] == '') $value['user'] = null;
        if ($value['adgrup'] == '') $value['adgrup'] = null;
        if ($value['usergrup'] == '') $value['usergrup'] = null;
        $db->query("INSERT INTO dle_cat_dostup (id_category, roly, user, ad_grup, user_grup, category, project) VALUE (
...
WHERE a.id_category = {$_POST['idcategory']} AND category = $cat $project
ORDER BY a.roly
";
