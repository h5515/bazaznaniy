...
if (trim($_POST['sertext']) == '')
  unset($_SESSION['sernews_id']);
...
if (!$tagson == "") {
  if ($tagcheck == 'true' && empty($_SESSION['sernews_id'])) {
    ...
    if (isset($_SESSION['sernews_id'])) {
      $news_id = $_SESSION['sernews_id'];
    }
...
if (isset($_SESSION['sernews_id'])) {
  $news_ids = implode(",", $_SESSION['sernews_id']);
  if (trim($news_ids) == '')
    $news_ids = 0;
  $news_ids = "AND news_id IN ($news_ids) ";
}
    ...
    foreach ($trin as $tru => $values) {
      ...
      if (!$b) {
        ...
        if ($pois != "") {
          ...
          if (!strpos(mb_strtolower($values), mb_strtolower($pois))) {
            $e = true;
          }
        }
...
//if ($_POST['tagcheck']=='false'||trim($_POST['sertext'])==''||trim($_POST['search'])!='')
echo implode(" ", $tags);
