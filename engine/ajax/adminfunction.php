	...
	if( count($ids) ) {
		...
		foreach ($ids as $id) {
			if($max_views[$id]['max_views'] AND ($max_views[$id]['views']+1) >= $max_views[$id]['max_views'] ) {
				@unlink( ENGINE_DIR . '/cache/system/'.$catl.'banners.php' );
			}
		}
	}
...
if ($_REQUEST['action'] == "savetheme") {
	...
	if( !is_dir( ENGINE_DIR . "/cache/system/'.$catl.'adminpanel" ) ) {
			
		@mkdir( ENGINE_DIR . "/cache/system/'.$catl.'adminpanel", 0777 );
		@chmod( ENGINE_DIR . "/cache/system/'.$catl.'adminpanel", 0777 );
	...
	if( !is_writable( ENGINE_DIR . "/cache/system/'.$catl.'adminpanel" ) ) {

		echo "{\"error\":\"{$lang['upload_error_1']} /engine/cache/system/'.$catl.'adminpanel/ {$lang['upload_error_2']}\"}";
		die();
		
	}

	if( file_exists( ENGINE_DIR . "/cache/system/'.$catl.'adminpanel/" . $file ) AND !is_writable( ENGINE_DIR . "/cache/system/'.$catl.'adminpanel/" . $file ) ) {
		
		$lang['stat_system'] = str_replace ("{file}", "/engine/cache/system/'.$catl.'adminpanel/" . $file, $lang['stat_system']);
	...
	if( count($theme) ) {
		...
		file_put_contents (ENGINE_DIR . "/cache/system/'.$catl.'adminpanel/" . $file, $theme, LOCK_EX);
		@chmod( ENGINE_DIR . "/cache/system/'.$catl.'adminpanel/" . $file, 0666 );
	
	} else {
		
		@unlink( ENGINE_DIR . "/cache/system/'.$catl.'adminpanel/" . $file );
