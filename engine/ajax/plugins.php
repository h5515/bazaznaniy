...
if($_POST['action'] == "updatefromurl") {
	...
	if( $type == "xml" ){
		...
		if(@copy($pluginurl, ENGINE_DIR . "/cache/system/".$catl . md5('uploads_plugin'.SECURE_AUTH_KEY) . ".zip")) {
			
			$_FILES['pluginfile']['tmp_name'] = ENGINE_DIR . "/cache/system/".$catl . md5('uploads_plugin'.SECURE_AUTH_KEY) . ".zip";
...
if ( isset($_SESSION['upload_plugins']['file']) AND isset($_SESSION['upload_plugins']['ftp']) ) {
	...
	if ( file_exists( ENGINE_DIR . "/cache/system/".$catl . md5('uploads_plugin'.SECURE_AUTH_KEY) . ".zip" ) ) {
...
function echo_error ($text, $unset = true) {
	...
	if($unset AND isset( $_SESSION['upload_plugins']['file'] ) ) {
		...
		@unlink(ENGINE_DIR . "/cache/system/".$catl . md5('uploads_plugin'.SECURE_AUTH_KEY) . ".zip");
	}
...
if( $type == "xml" ){
	...
	if(count($no_access) AND !isset( $_SESSION['upload_plugins']['ftp'] )) {
		...
		else $uploaded=@move_uploaded_file($_FILES['pluginfile']['tmp_name'], ENGINE_DIR . "/cache/system/".$catl . md5('uploads_plugin'.SECURE_AUTH_KEY) . ".zip");
