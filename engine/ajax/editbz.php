...
if ( !$allow_addnews ) {
  ...
  if ( isset( $_REQUEST[ 'mod' ] )AND $_REQUEST[ 'mod' ] == "addnews"
    ...
    if ( !$stop ) {
      ...
      if ( $found ) {
        $db->query("DELETE FROM dle_draft WHERE id={$id} and user='{$atr}'");
      ...
      // echo $lang['add_ok']."  ".$msg. $add_url. $lang['add_or'];"{$msg} {$add_url} {$lang['add_or']} <a href=\"{$config['http_home_url']}\">{$lang['all_prev']}</a>"
      $san = 'susu' . '<div id="modal-iframe" data-iziModal-group="grupo1">
                    <button data-izimodal-close="">Close</button>
                    </div>';
      msgbox( $lang[ 'add_ok' ], $lang[ 'add_ok_1' ] );
...
function preview(){";
    ...
    $script .= "if(document.entryform.title.value == ''){ DLEalert('$lang[add_err_7]', dle_info); }
        ...
        document.entryform.mod.value='preview';document.entryform.action='{$PHP_SELF}?do=preview&newsid={$id}';document.entryform.target='prv'
