...
$rows = $db->super_query( "SELECT id, title, full_story, autor, date, category, tags FROM " . PREFIX . "_post WHERE id='" . $ids . "' LIMIT 0,1" );
...
if ( $tags != '' ) {
  $rows[ 'template' ] = str_replace( "{%tags%}", $tags, $rows[ 'template' ] );
  $rows[ 'template' ] = str_replace( "[tags]", '', $rows[ 'template' ] );
  $rows[ 'template' ] = str_replace( "[/tags]", '', $rows[ 'template' ] );
} else {
  $rows[ 'template' ] = preg_replace( "'\\[tags].*?\\[/tags\\]'si", '', $rows[ 'template' ] );
}
...
$db->query( "SELECT email, allow_mail FROM " . USERPREFIX . "_users" . $where . " ORDER BY user_id DESC" );
...
echo '<div align="center">' . $lang[ 'mail_send_ok' ] . '<div>';
