...
if ( $area == "template" ) {
	...
	if ( !$user_group[$member_id['user_group']]['admin_static'] ) die( "Hacking attempt1!" );
...
if ( $area == "adminupload" ) {
	...
	if ( $member_id['user_group'] != 1 ) die( "Hacking attempt3!" );
...
if ( $news_id AND $area != "template" AND $area != "comments" ) {
	...
	if ( !$row['id'] ) die( "Hacking attempt4!" );

	//if ( !$user_group[$member_id['user_group']]['allow_all_edit'] AND $row['autor'] != $member_id['name'] ) die( "Hacking attempt5!" );
  //  if ( !$user_group[$member_id['user_group']]['allow_all_edit']) die( "Hacking attempt5!  ".$user_group[$member_id['user_group']]['allow_all_edit'] );
	...
	if( $user_group[$member_id['user_group']]['allow_file_upload'] AND is_array($_POST['files']) AND count( $_POST['files'] ) ) {
		...
		foreach ( $_POST['files'] as $file ) {
			...
			if ( $row['id'] ) {
				...
				if( trim($file) == ".htaccess") die("Hacking attempt9!");	
	...
	if( $user_group[$member_id['user_group']]['admin_static'] AND is_array($_POST['static_files']) AND count( $_POST['static_files'] ) ) {
		...
		foreach ( $_POST['static_files'] as $file ) {
			...
			if( $row['id'] and $row['onserver'] ) {
				...
				if( trim($file) == ".htaccess") die("Hacking attempt10!");	
...
jQuery(function($){
	...
	var uploader = new qq.FileUploader({
		...
		//allowedExtensions: ['{$simple_ext}'], расширение файла
		...
		onComplete: function(id, fileName, response){
						...
						if ( response.success ) {
							...
							$('#uploadfile-'+id+' .qq-status').html('{$lang['media_upload_st9']}..'+'{$area}');
