...
switch ( $do ) {
	...
	case "addnews" :
		...
		break;
	case "editbz" :
		include (DLEPlugins::Check(ENGINE_DIR . '/modules/editbz.php'));
		break;
	...
	default :
		...
		$sql_select = "SELECT p.id, p.autor, p.edit_autor as editor, p.date, p.short_story, p.full_story as full_story, p.xfields, p.title, p.category, p.alt_name, p.comm_num, p.allow_comm, p.fixed, p.tags, e.news_read, e.allow_rate, e.rating, e.vote_num, e.votes, e.view_edit, e.editdate, e.reason FROM " . PREFIX . "_post p {$cat_join}LEFT JOIN " . PREFIX . "_post_extras e ON (p.id=e.news_id) WHERE {$stop_list}userinfo=1 AND allow_main=1" . $where_date . " ORDER BY " . $fixed . $news_sort_by . " " . $news_direction_by . " LIMIT " . $cstart . "," . $config['news_number'];

		$sql_count = "SELECT COUNT(*) as count FROM " . PREFIX . "_post {$cat_join_count}WHERE {$stop_list}approve=1 AND arhiv=0 AND allow_main=1" . $where_date;
		$sql_news = "";
		...
		if ($do == "cat" and $category != '' and $subaction == '') {
			...
			if ($view_template == "rss") {
				...
				$sql_select = "SELECT p.id, p.autor, p.edit_autor as editor, p.date, p.short_story, p.full_story, p.xfields, p.approve,p.arhiv, p.title, p.category, p.alt_name, p.comm_num, p.allow_comm, p.fixed, p.tags, e.news_read, e.allow_rate, e.rating, e.vote_num, e.votes, e.view_edit, e.editdate, e.reason, q.id_news as id_news FROM " . PREFIX . "_post p {$join_category}{$join_read}LEFT JOIN " . PREFIX . "_post_extras e ON (p.id=e.news_id) WHERE {$where_category}approve=1 AND arhiv=0" . $where_date . " ORDER BY " . $fixed . $news_sort_by . " " . $news_direction_by . " LIMIT " . $cstart . "," . $config['news_number'];
                
                //echo $sql_select;

                	/*$sql_select = "SELECT p.id, p.autor, p.date, p.short_story, p.full_story, p.xfields, p.approve, p.title, p.category, p.alt_name, p.comm_num, p.allow_comm, p.fixed, p.tags, e.news_read, e.allow_rate, e.rating, e.vote_num, e.votes, e.view_edit, e.editdate, e.editor, e.reason FROM " . PREFIX . "_post p {$join_category}LEFT JOIN " . PREFIX . "_post_extras e ON (p.id=e.news_id) WHERE {$where_category}approve=1" . $where_date . " ORDER BY " . $fixed . $news_sort_by . " " . $news_direction_by . " LIMIT " . $cstart . "," . $config['news_number'];*/
                
				/*$sql_select = "SELECT p.id, p.autor, p.date, p.short_story, CHAR_LENGTH(p.full_story) as full_story, p.xfields, p.approve, p.title, p.category, p.alt_name, p.comm_num, p.allow_comm, p.fixed, p.tags, e.news_read, e.allow_rate, e.rating, e.vote_num, e.votes, e.view_edit, e.editdate, e.editor, e.reason FROM " . PREFIX . "_post p {$join_category}LEFT JOIN " . PREFIX . "_post_extras e ON (p.id=e.news_id) WHERE {$where_category}approve=1" . $where_date . " ORDER BY " . $fixed . $news_sort_by . " " . $news_direction_by . " LIMIT " . $cstart . "," . $config['news_number'];*/
				
				
				$sql_count = "SELECT COUNT(*) as count FROM " . PREFIX . "_post p {$join_category}WHERE {$where_category}approve=1 AND arhiv=0" . $where_date;
			...
			$sql_count = "SELECT COUNT(*) as count FROM " . PREFIX . "_post {$cat_join_count}WHERE {$stop_list}approve=1 AND arhiv=0" . $where_date;
	...
	if (!$_GET['cats']==''){
	$ids = getSubCatList($_GET['cats']  );
		
	$aviable = array();
    $aviable = explode( ',', $ids );
	$catser = " p.category regexp '[[:<:]](" . implode ( '|', $aviable ) . ")[[:>:]]' and ";
	}
			...
			$tag = @$db->safesql ( $tag );
						...
						$sql_select = "SELECT p.id, p.autor, p.edit_autor as editor, p.date, p.short_story, p.full_story as full_story, p.xfields, p.title, p.category, p.alt_name, p.comm_num, p.allow_comm, p.fixed, p.tags, e.news_read, e.allow_rate, e.rating, e.vote_num, e.votes, e.view_edit, e.editdate, e.reason FROM " . PREFIX . "_post p INNER JOIN " . PREFIX . "_tags t on (t.news_id=p.id) {$cat_join}LEFT JOIN " . PREFIX . "_post_extras e ON (p.id=e.news_id) WHERE $catser {$stop_list}t.tag = '{$tag}' AND p.approve=1" . $where_date .  " ORDER BY " . $news_sort_by . " " . $news_direction_by . " LIMIT " . $cstart . "," . $config['news_number'];
			$sql_count = "SELECT COUNT(*) as count FROM " . PREFIX . "_post p INNER JOIN " . PREFIX . "_tags t on (t.news_id=p.id) {$cat_join}WHERE {$stop_list}t.tag = '{$tag}' AND approve=1 AND arhiv=0" . $where_date;
		...
		/*	$sql_select = "SELECT p.id, p.autor, p.date, p.short_story, CHAR_LENGTH(p.full_story) as full_story, p.xfields, p.title, p.category, p.alt_name, p.comm_num, p.allow_comm, p.fixed, p.tags, e.news_read, e.allow_rate, e.rating, e.vote_num, e.votes, e.view_edit, e.editdate, e.editor, e.reason FROM " . PREFIX . "_post p INNER JOIN " . PREFIX . "_tags t on (t.news_id=p.id) {$cat_join}LEFT JOIN " . PREFIX . "_post_extras e ON (p.id=e.news_id) WHERE {$stop_list}t.tag = '{$tag}' AND p.approve=1" . $where_date . " ORDER BY " . $news_sort_by . " " . $news_direction_by . " LIMIT " . $cstart . "," . $config['news_number'];
			...
			if ( $xfname ) {
				...
				$sql_select = "SELECT p.id, p.autor, p.edit_autor as editor, p.date, p.short_story, p.full_story as full_story, p.xfields, p.title, p.category, p.alt_name, p.comm_num, p.allow_comm, p.fixed, p.tags, e.news_read, e.allow_rate, e.rating, e.vote_num, e.votes, e.view_edit, e.editdate, e.reason FROM " . PREFIX . "_post p INNER JOIN " . PREFIX . "_xfsearch xf on (xf.news_id=p.id) {$cat_join}LEFT JOIN " . PREFIX . "_post_extras e ON (p.id=e.news_id) WHERE {$stop_list}xf.tagname = '{$xfname}' AND xf.tagvalue='{$xf}' AND p.approve=1" . $where_date . " ORDER BY " . $news_sort_by . " " . $news_direction_by . " LIMIT " . $cstart . "," . $config['news_number'];
				$sql_count  = "SELECT COUNT(*) as count FROM " . PREFIX . "_post p INNER JOIN " . PREFIX . "_xfsearch xf on (xf.news_id=p.id) {$cat_join}WHERE {$stop_list}xf.tagname = '{$xfname}' AND xf.tagvalue='{$xf}' AND approve=1 AND arhiv=0" . $where_date;
			...
			if ($member_id['name'] == $user OR $user_group[$member_id['user_group']]['allow_all_edit']) {
				...
				$sql_select = "SELECT p.id, p.autor, p.edit_autor as editor, p.date, p.short_story, p.full_story as full_story, p.xfields, p.title, p.category, p.alt_name, p.comm_num, p.allow_comm, p.fixed, p.tags, p.approve, e.news_read, e.allow_rate, e.rating, e.vote_num, e.votes, e.view_edit, e.editdate, e.reason, q.id_news as id_news FROM " . PREFIX . "_post p{$join_read} LEFT JOIN " . PREFIX . "_post_extras e ON (p.id=e.news_id) WHERE autor = '{$user}' AND approve=0 ORDER BY " . $news_sort_by . " " . $news_direction_by . " LIMIT " . $cstart . "," . $config['news_number'];
			...
			if ($view_template == "rss") {
				...
				$sql_count = "SELECT COUNT(*) as count FROM " . PREFIX . "_post {$cat_join_count}WHERE {$stop_list}autor = '$user' AND approve=1 AND arhiv=0" . $where_date;
			}
			...
			$sql_count = "SELECT COUNT(*) as count FROM " . PREFIX . "_post {$cat_join_count}WHERE {$stop_list}approve=1 AND arhiv=0 AND date between '$lasttime' and '$thistime'";
			
			$config['allow_cache'] = false;
		
        } elseif ( $subaction == 'approve' ) {
       
      // ################ show all unread approve #################
      if ( $cstart ) {
        $cstart = $cstart - 1;
        $cstart = $cstart * $config[ 'news_number' ];
      }

      $url_page = $config[ 'http_home_url' ] . "approve";
      $user_query = "subaction=approve";

      if ( $config[ 'allow_alt_url' ] )$canonical = $url_page . "/";
      else $canonical = $PHP_SELF . "?subaction=approve";
      ...
      if ( isset( $_SESSION[ 'dle_sort_approve' ] ) )$news_sort_by = $_SESSION[ 'dle_sort_approve' ];
      if ( isset( $_SESSION[ 'dle_direction_approve' ] ) )$news_direction_by = $_SESSION[ 'dle_direction_approve' ];
      ...
      if ( !$catlist == "0" ) {
        ...
        //$gid = $id = get_idcategories( $id );
        $catname = get_cat_name( $id );

      $sql_select = "SELECT p.id, p.autor, p.edit_autor as editor, p.date, p.short_story, p.full_story,p.approve, p.xfields, p.title, p.category, p.alt_name, p.comm_num, p.allow_comm, p.fixed, p.tags, e.news_read, e.allow_rate, e.rating, e.vote_num, e.votes, e.view_edit, e.editdate, e.reason FROM " . PREFIX . "_post p LEFT JOIN dle_post_extras e ON (p.id=e.news_id) WHERE approve=0" . $tcat . " ORDER by date desc";
        $sql_count = "SELECT COUNT(*) as count FROM " . PREFIX . "_post {$cat_join_count}WHERE {$stop_list}approve=0 " . $tcat . " AND date between '$lasttime' and '$thistime'";
      $config[ 'allow_cache' ] = false;
			...
			$sql_count = "SELECT COUNT(*) as count FROM " . PREFIX . "_post {$cat_join_count}WHERE {$stop_list}symbol = '$catalog' AND approve=1 AND arhiv=0 " . $where_date;
			...
			if ($year != '' and $month == '' and $day == '') {
				...
				$sql_count = "SELECT COUNT(*) as count FROM " . PREFIX . "_post {$cat_join_count}where {$stop_list}date >= '{$year}-01-01'AND date < '{$year}-01-01' + INTERVAL 1 YEAR AND approve=1 AND arhiv=0 " . $where_date;
			}
			...
			if ($year != '' and $month != '' and $day == '') {
				...
				$sql_count = "SELECT COUNT(*) as count FROM " . PREFIX . "_post {$cat_join_count}where {$stop_list}date >= '{$year}-{$month}-01'AND date < '{$year}-{$month}-01' + INTERVAL 1 MONTH AND approve=1 AND arhiv=0" . $where_date;
			}
			...
			if ($year != '' and $month != '' and $day != '' and $subaction == '') {
				...
				$sql_count = "SELECT COUNT(*) as count FROM " . PREFIX . "_post {$cat_join_count}WHERE {$stop_list}date >= '{$year}-{$month}-{$day}' AND date < '{$year}-{$month}-{$day}' + INTERVAL 24 HOUR AND approve=1 AND arhiv=0" . $where_date;
			...
			if ($subaction != '' OR $newsid) {
                ...
                if ($_GET['vivid']=="ok"){
                    $tabl = true;
                    $sql_news = "SELECT * FROM " . PREFIX . "_post_arhiv WHERE  ids = '{$newsid}'";
                } else 
                {
                    $tabl = false;
                }
               // echo $tabl;
				...
				else if (!$tabl) $sql_news = "SELECT *,q.id_news FROM " . PREFIX . "_post ".$join_read." LEFT JOIN " . PREFIX . "_post_extras ON (" . PREFIX . "_post.id=" . PREFIX . "_post_extras.news_id) WHERE  id = '{$newsid}'";
...
elseif ($subaction == 'approve') $nam_e = $lang['title_approve']." ".$catname;
elseif ($do == 'stats') $nam_e = $lang['title_stats'];
elseif ($do == 'addnews') $nam_e = $lang['title_addnews'];
elseif ($do == 'editbz') $nam_e = $lang['title_editbz'];
