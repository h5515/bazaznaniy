...
switch ( $do ) {
	...
	$catser = " p.category regexp '[[:<:]](" . implode ( '|', $aviable ) . ")[[:>:]]' and ";
      ...
      $sql_select = "SELECT p.id, p.autor, p.date, p.short_story, p.full_story,p.approve, p.xfields, p.title, p.category, p.alt_name, p.comm_num, p.allow_comm, p.fixed, p.tags, e.news_read, e.allow_rate, e.rating, e.vote_num, e.votes, e.view_edit, e.editdate, e.editor, e.reason FROM " . PREFIX . "_post p LEFT JOIN dle_post_extras e ON (p.id=e.news_id) WHERE approve=0" . $tcat . " ORDER by date desc";
