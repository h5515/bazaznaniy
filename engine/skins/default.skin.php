	...
	while ($row = $db->get_array()) {
		...
		if (count(explode("@", $row['foto'])) == 2) {
			...
			if ($row['foto']) {
				...
				if (strpos($row['foto'], "//") === 0) $pm_avatar = "http:" . $row['foto'];
				else $pm_avatar = $row['foto'];
...
HTML;
	...
	if (isset($_COOKIE['dbname']) && $_COOKIE['dbname'] != $gl_bd) {
		unset($options['user'][5]);
	}
	...
	foreach ($options as $sub_options) {
		...
		$icon = "wrench";
		if ($subs == 1) {
			$menu_item_header = $lang['opt_s_acc'];
			$icon = "user";
		}
		if ($subs == 2) {
			$menu_item_header = $lang['opt_s_tem'];
			$icon = "laptop";
		}
		if ($subs == 3) {
			$menu_item_header = $lang['opt_s_fil'];
			$icon = "leaf";
		}
		if ($subs == 4) {
			$menu_item_header = $lang['opt_s_oth'];
			$icon = "link";
		}
		if ($subs == 5) {
			$menu_item_header = $lang['admin_other_section'];
			$icon = "list-alt";
		}
...
$icon = '<img src="images/superadmin.jpg">';
if (isset($_COOKIE['dbname']) && $_COOKIE['dbname'] != $gl_bd) {
	$bd2 = new db;
	$bd2->connect(DBUSER, DBPASS, $gl_bd, DBHOST);
	$rows = $bd2->super_query("SELECT Name, id_cat FROM " . USERPREFIX . "_project WHERE Project = '{$_COOKIE['dbname']}'");
	$title = $rows['Name'];
	$bd = new db;
	$rows = $bd->super_query("SELECT icon FROM " . USERPREFIX . "_category WHERE id = {$rows['id_cat']}");
	$icon = '<img src="' . $rows['icon'] . '">';
	$bd->free();
	$bd2->free();
} else
	$title = 'Админ панель';
...
<head>
	<meta charset="utf-8">
	<title>{$title} - {$lang['skin_title']}</title>
...
<div id="loading-layer" class="shadow-depth3"><i class="fa fa-spinner fa-spin"></i></div>
	<div class="navbar navbar-inverse bg-primary-700">
		<div class="navbar-header">
			<a class="navbar-brand" href="?mod=main"><div class="tetlicon">{$icon}{$title}</div></a>
...
$skin_footer = <<<HTML
					<div class="footer text-muted text-size-small">
						Автор &copy; Александр Хованский, год создания 2020 - 2021.
					</div>
				</div>
			</div>
		</div>
