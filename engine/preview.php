...
if (isset($_GET['newsid'])) {
	$id = $_GET['newsid'];
	if (strpos($full_story, "[attachment=") !== false) {
		$full_story = show_attach($full_story, $id);
	}
}

$full_story = preg_replace( "'\[metka=(.*?)\]'si", "<span id='per\\1'></span>", $full_story);
$full_story = preg_replace( "'\[url_metka=(.*?)\](.*?)\[/url_metka\]'si", "<a href='#' class='urlmetka' data-url='\\1'>\\2</a>",$full_story);
