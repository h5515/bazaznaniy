...
if( $config['allow_comments_wysiwyg'] == 1 ) {
	...
	if ($user_group[$member_id['user_group']]['allow_up_image']) {
		...
		$image_upload_params = "imageUpload: true,";
	}
...
/*	$onload_scripts[] = <<<HTML
      ...
      $('#comments').froalaEditor({
		...
		imageUploadParams: { "subaction" : "upload", "news_id" : "{$p_id}", "area" : "short_story", "author" : "{$p_name}", "mode" : "quickload", "user_hash" : "{$dle_login_hash}"},
        imageMaxSize: {$config['max_up_size']} * 1024,
        imagePaste: true,

		
        toolbarButtonsXS: ['bold', 'italic', 'underline', 'strikeThrough', '|', 'align', 'formatOL', 'formatUL', '|', {$link_icon} 'emoticons', '|', 'dlehide', 'dlequote', 'dlespoiler', '{$implugin}', {$image_upload}'fullscreen'],

        toolbarButtonsSM: ['bold', 'italic', 'underline', 'strikeThrough', '|', 'align', 'formatOL', 'formatUL', '|', {$link_icon} 'emoticons', '|', 'dlehide', 'dlequote', 'dlespoiler', '{$implugin}', {$image_upload}'fullscreen'],

        toolbarButtonsMD: ['bold', 'italic', 'underline', 'strikeThrough', '|', 'align', 'formatOL', 'formatUL', '|', {$link_icon} 'emoticons', '|', 'dlehide', 'dlequote', 'dlespoiler','{$implugin}',{$image_upload}'fullscreen'],

        toolbarButtons: ['bold', 'italic', 'underline', 'strikeThrough', '|', 'align', 'formatOL', 'formatUL', '|', {$link_icon} 'emoticons', '|', 'dlehide', 'dlequote', 'dlespoiler','{$implugin}',{$image_upload}'fullscreen']
