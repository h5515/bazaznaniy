	...
	while ( $maxdays > $cal_day ) {
		...
		if( isset( $events[$cal_day] ) ) {
			...
			if( $weekday == '5' or $weekday == '6' ) {
				...
				else $buffer .= '<td '.(($cal_pos==$cur_date)?' class="day-active day-current" ':' class="day-active" ').'><a class="day-active" href="#" onClick="filtered(this,\'Архив - '.date( "d", $events[$cal_day] ).'.'.date( "m", $events[$cal_day] ).'.'.date( "Y", $events[$cal_day] ).'\');return false;" title="' . $lang['cal_post'] . ' ' . $date['title'] . '">' . $cal_day . '</a></td>';
				...
				else $buffer .= '<td '.(($cal_pos==$cur_date)?' class="day-active-v day-current" ':' class="day-active-v" ').'><a class="day-active-v" href="#" onClick="filtered(this,\'Архив - '.date( "d", $events[$cal_day] ).'.'.date( "m", $events[$cal_day] ).'.'.date( "Y", $events[$cal_day] ).'\');return false;" title="' . $lang['cal_post'] . ' ' . $date['title'] . '">' . $cal_day . '</a></td>';
...
if( $config['allow_calendar'] ) {
	...
	$tpl->result['calendar'] = dle_cache( "calendar".$category_id, $cache_id );
	
		$cots = getSubCatList($category_id);
		$aviable = array();
        $aviable = explode( ',', $cots );
		$categor = " AND category regexp '[[:<:]](" . implode ( '|', $aviable ) . ")[[:>:]]' ";
	...
	if( ! $tpl->result['calendar'] ) {
		...
		if( $year != '' AND $month != '' ) {
			...
			$sql = "SELECT DISTINCT DAYOFMONTH(date) as day FROM " . PREFIX . "_post WHERE date >= '{$year}-{$month}-01' AND date < '{$year}-{$month}-01' + INTERVAL 1 MONTH" . $approve . $where_date. $categor.$arhiv;
			...
			$sql = "SELECT DISTINCT DAYOFMONTH(date) as day FROM " . PREFIX . "_post WHERE date >= '{$this_year}-{$this_month}-01' AND date < '{$this_year}-{$this_month}-01' + INTERVAL 1 MONTH AND approve=1" . $where_date. $categor.$arhiv;
		...
		create_cache( "calendar".$category_id, $tpl->result['calendar'], $cache_id );
	}
...
if( $config['allow_archives'] ) {
	...
	$tpl->result['archive'] = dle_cache( "archives".$category_id, $config['skin'] );
	...
	if( ! $tpl->result['archive'] ) {
		...
		$db->query( "SELECT DATE_FORMAT(date,'%b %Y') AS m_date, MAX(date) AS max, COUNT(id) AS cnt FROM " . PREFIX . "_post WHERE approve=1".$categor.$arhiv . $where_date . " GROUP BY m_date ORDER BY max desc" );
		...
		while ( $row = $db->get_row() ) {
			...
			else $news_archive[] = "<a class=\"archives\" href=\"#\" onClick=\"filtered(this,'Архив - ".$arch_url[0] .".".$arch_url[1] ."');return false;\" title='" . $lang['cal_post'] . " " . $arch_title['ru'] . "'><b>" . $arch_title['ru'] . " (" . $row['cnt'] . ")</b></a>";
	/*		else $news_archive[] = "<a class=\"archives\" href=\"$PHP_SELF?year=$arch_url[1]&amp;month=$arch_url[0]\" onClick=\"filtered(this,'Архив - ".$arch_url[0] .".".$arch_url[1] ."');return false;\" title='" . $lang['cal_post'] . " " . $arch_title['ru'] . "'><b>" . $arch_title['ru'] . " (" . $row['cnt'] . ")</b></a>";*/
		...
		create_cache( "archives".$category_id, $tpl->result['archive'], $config['skin'] );
	}
