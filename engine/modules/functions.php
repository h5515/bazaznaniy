...
{
	$id = intval($id);
	$sql = "SELECT name FROM " . PREFIX . "_category WHERE id = '" . $id . "'";
	$db = new db;
...
function langdate($format, $stamp, $servertime = false, $custom = false)
{
	...
	if (is_array($custom)) $locallangdate = $customlangdate;
...
HTML;
		...
		if ($row['num']) $rating = number_format($row['rating'], 0, ',', ' ');
		else $rating = 0;
...
function custom_print($matches = array())
{
	...
	if ($config['allow_cache']) $short_news_cache = true;
	else $short_news_cache = false;
...
function check_category($matches = array())
{
	...
	if ($matches[1] == "category" or $matches[1] == "catlist") $action = true;
	else $action = false;
...
function get_categories($id, $separator = " &raquo;")
{
	...
	if ($config['allow_alt_url']) $list = "<a href=\"" . $config['http_home_url'] . get_url($id) . "/\">{$cat_info[$id]['name']}</a>";
		...
		if ($_GET['stroka'] == "ok")
			...
			$list = "<a href=\"$PHP_SELF?do=cat&amp;{$prjlink}category={$cat_info[$id]['alt_name']}\">{$cat_info[$id]['name']}</a>";
	}
	while ($parent_id) {
...
function get_breadcrumbcategories($id, $separator = "&raquo;", $last_link = true)
{
	...
	if ($last_link) {
		...
		else $list[] = "<span itemprop=\"itemListElement\" itemscope itemtype=\"https://schema.org/ListItem\"><meta itemprop=\"position\" content=\"{pos}\"><a href=\"$PHP_SELF?do=cat&amp;{$prjlink}category={$cat_info[$id]['alt_name']}\" itemprop=\"item\"><span itemprop=\"name\">{$cat_info[$id]['name']}</span></a></span>";
	} else {
	...
	while ($parent_id) {
		...
		else $list[] = "<span itemprop=\"itemListElement\" itemscope itemtype=\"https://schema.org/ListItem\"><meta itemprop=\"position\" content=\"{pos}\"><a href=\"$PHP_SELF?do=cat&amp;{$prjlink}category={$cat_info[$parent_id]['alt_name']}\" itemprop=\"item\"><span itemprop=\"name\">{$cat_info[$parent_id]['name']}</span></a></span>" . $separator;
...
function news_sort($do)
{
	...
	if (!$config['allow_comments']) {
		unset($allowed_sort[3]);
		unset($soft_by_array['comm_num']);
	}
	...
	foreach ($soft_by_array as $value) {   //dle_change_sort

		$sort[] = "<li" . $value['image'] . "><a href=\"#\" onclick=\"formsendsort('{$value['value']}','{$value['direction']}',this); return false;\">" . $value['name'] . "</a></li>";
	}
...
function check_static($matches = array())
{
	...
	if ($matches[1] == "static") $action = true;
	else $action = false;
...
function get_ip()
{
	...
	if ($config['own_ip']) $ip = $_SERVER[$config['own_ip']];
...
function deletenewsbyid($id, $arc)
{
	global $config, $db;
	...
	if ($arc) {
		$db->query("DELETE FROM " . PREFIX . "_post_arhiv WHERE ids='{$id}'");
	} else {
		...
		$db->query("DELETE FROM " . PREFIX . "_post_arhiv WHERE id='{$id}'");
