...
if( count( $informers ) ) {
	...
	foreach ( $temp as $key => $value ) {
		...
		if ( $buffer !== false) {
			...
			$file_date = @filemtime( ENGINE_DIR.'/cache/'.$catl.'informer_'.$value['id'].'_'.md5($config['skin']).'.tmp' );
			...
			if ( $file_date ) {
				...
				if (date ( "d-H", $file_date ) != date ( "d-H" )) {
					...
					$buffer = false;
					@unlink( ENGINE_DIR.'/cache/'.$catl.'informer_'.$value['id'].'_'.md5($config['skin']).'.tmp' );
