...
if ($config['allow_alt_url'] and !$config['seo_type']) $cprefix = "full";
...
//AND !$row['approve']
//if( $row['id']  AND $member_id['name'] != $row['autor'] AND !$user_group[$member_id['user_group']]['allow_all_edit'] ) $perm = 0;
			...
			if ($element and $cat_info[$element]['id']) {
				...
				if ($config['allow_alt_url']) $my_cat_link[] = "<a href=\"" . $config['http_home_url'] . get_url($element) . "/\">{$cat_info[$element]['name']}</a>";
					...
					if ($_GET['stroka'] == "ok")
						...
						$my_cat_link[] = "<a href=\"$PHP_SELF?do=cat&amp;{$prjlink}category={$cat_info[$element]['alt_name']}\">{$cat_info[$element]['name']}</a>";
				}
			}
		}
...
if ($row['id'] and  $perm) {
	...
	if ($view_modal == "modal") {
		//echo "111".$view_modal;
		$tpl->load_template('fullstory2.tpl');
	} else
	...
	if ($vdt == 1) {
		...
		$tpl->set_block("'\\[no-histor\\](.*?)\\[/no-histor\\]'si", "");
		$tpl->set('[histor]', "");
		$tpl->set('[/histor]', "");
		$prosmotrbol = true;
		include(DLEPlugins::Check(ENGINE_DIR . '/modules/history.php'));
		$tpl->set('{history}', $tpl->result['history']);
	} else {
		$tpl->set('[no-histor]', "");
		$tpl->set('[/no-histor]', "");
		$sqls = "SELECT ids, autor, date, approve FROM dle_post_arhiv WHERE id = {$row['id']} ";
	...
	if (stripos($tpl->copy_template, "{next-") !== false or stripos($tpl->copy_template, "{prev-") !== false) {
		...
		if (!is_array($prev_next)) {
			...
			$row_link = $db->super_query("SELECT id, date, title, category, alt_name FROM " . PREFIX . "_post WHERE category = '{$row['category']}' AND date >= '{$row['date']}'{$where_date} AND id != '{$row['id']}' AND approve = '1' AND arhiv = '0' ORDER BY date ASC LIMIT 1");
			...
			$row_link = $db->super_query("SELECT id, date, title, category, alt_name FROM " . PREFIX . "_post WHERE category = '{$row['category']}' AND date <= '{$row['date']}'{$where_date} AND id != '{$row['id']}' AND approve = '1' AND arhiv = '0' ORDER BY date DESC LIMIT 1");
		...
		if ($anzahl_seiten > 1) {
			...
			if ($news_page < $anzahl_seiten) {
				...
				if ($config['allow_alt_url']) {
					...
					$nextpage = "<a href=\"$PHP_SELF?newsid=" . $row['id'] . $prjlink . "&amp;news_page=" . $pages . "\">";
				}
			...
			if ($news_page > 1) {
				...
				if ($config['allow_alt_url']) {
					...
					if ($pages == 1) $prevpage = "<a href=\"" . $full_link . "\">";
					else $prevpage = "<a href=\"$PHP_SELF?newsid=" . $row['id'] . $prjlink . "&amp;news_page=" . $pages . "\">";
				}
			...
			if ($anzahl_seiten <= 10) {
				...
				if ($end >= $anzahl_seiten - 1) $nav_prefix = "";
				...
				if ($news_page != $anzahl_seiten) {
					...
					else $listpages .= $nav_prefix . "<a href=\"$PHP_SELF?newsid=" . $row['id'] . $prjlink . "&amp;news_page=" . $anzahl_seiten . "\">{$anzahl_seiten}</a>";
				} else
	...
	if ($config['allow_tags'] and $row['tags']) {
		...
		foreach ($row['tags'] as $value) {
			...
			else $tags[] = "<span class='clouds_xsmall'><a href=\"#\">" . $value . "</span></a>";
			//else $tags[] = "<span class='clouds_xsmall'><a href=\"$PHP_SELF?do=tags&amp;tag=" . rawurlencode( $url_tag ) . "\">" . $value . "</span></a>";
		}
	...
	if ($category_id) {
		...
		else $tpl->set('{category-url}', "$PHP_SELF?do=cat&{$prjlink}category={$cat_info[$category_id]['alt_name']}");
	...
	if ($row['allow_rate'] and !$redact) {
		...
		$tpl->set('[rating]', "");
		$tpl->set('[/rating]', "");
		$tpl->set_block("'\\[redact\\](.*?)\\[/rating\\]'si", "");
	...
	if ($allow_userinfo and !$user_group[$member_id['user_group']]['allow_edit'] and $user_group[$member_id['user_group']]['allow_adds']) {
		...
		$tpl->set('[edit]', "<a onclick=\"return dropdownmenu(this, event, MenuNewsBuild('" . $ids . "', 'full', '" . $row['arhiv'] . "', '" . $row['approve'] . "'), '170px')\" href=\"#\">");
		$tpl->set('[/edit]', "</a>");
	...
	if ($config['related_news'] and $view_template != "print") {
		...
		if ($allow_full_cache) $related_buffer = dle_cache("related", $row['id'] . $config['skin'], true);
		else $related_buffer = false;
		...
		if ($related_buffer === false) {
			...
			if ($row['related_ids']) {
				...
				$db->query("SELECT id, date, short_story, xfields, title, category, alt_name FROM " . PREFIX . "_post WHERE id IN({$row['related_ids']}) AND approve=1 AND arhiv=0 ORDER BY FIND_IN_SET(id, '{$row['related_ids']}') LIMIT " . $config['related_number']);
				$first_show = false;
			} else {
			...
			while ($related = $db->get_row()) {
				...
				if ($config['allow_alt_url']) {
					...
					$rel_full_link = $config['http_home_url'] . "index.php?newsid=" . $related['id'] . $prjlink;
				}
				...
				if (stripos($related['short_story'], "[hide") !== false) {
					...
					$related['short_story'] = preg_replace_callback(
						"#\[hide(.*?)\](.+?)\[/hide\]#is",
	...
	if ($_GET['stroka'] == 'ok') {
		if ($row['id_news'] == '') {
			$prohit_sit = ';top.postMessage(\'st0' . $row['id'] . '\', \'*\')';
		} else {
			$prohit_sit = ';top.postMessage(\'st1' . $row['id'] . '\', \'*\')';
		}
	}

	if (!$_GET['vivid'] == "ok") {
		if ($row['id_news'] == '') {
			$prochit = '<br><div align="center" class="prochit"><span><a href="#" onClick="procitano(' . $row['id'] . ',this)' . $prohit_sit . ';return false;" class="button21"><img src="images\voskl.png"/><b>Не прочитано!</b></a></span></div><br>';
		} else {
			$prochit = '<br><div align="center" class="prochit"><span><a href="#" onClick="procitano(' . $row['id'] . ',this)' . $prohit_sit . ';return false;" class="button21"><img src="images\galka.png"/>Прочитано!</a></span></div><br>';
		}
	}

	$tpl->set('{full-story}', $row['full_story'] . $prochit);
...
if ($allow_comments and $news_found) {
	...
	if ($comments_num > 0) {
		...
		if ($config['comm_msort'] == "" or $config['comm_msort'] == "ASC") $comm_msort = "ASC";
		else $comm_msort = "DESC";
		...
		$comments->query = "SELECT " . PREFIX . "_comments.id, post_id, " . PREFIX . "_comments.user_id, date, autor as gast_name, " . PREFIX . "_comments.email as gast_email, text, ip, is_register, " . PREFIX . "_comments.rating, " . PREFIX . "_comments.vote_num, " . PREFIX . "_comments.parent, name, {$gl_bd}.dle_users.email, news_num, comm_num, user_group, lastdate, reg_date, signature, foto, fullname, land, xfields FROM " . PREFIX . "_comments LEFT JOIN {$gl_bd}.dle_users ON " . PREFIX . "_comments.autor=" . USERPREFIX . "_users.name WHERE " . PREFIX . "_comments.post_id = '$news_id'" . $where_approve . " ORDER BY " . PREFIX . "_comments.id " . $comm_msort;
	...
	if ($user_group[$member_id['user_group']]['allow_addc'] and $config['allow_comments'] and $allow_add and ($member_id['restricted'] != 2 and $member_id['restricted'] != 3)) {
		...
		if ($config['allow_subscribe'] and $is_logged and $user_group[$member_id['user_group']]['allow_subscribe']) $allow_subscribe = true;
