...
if ($config['top_number'] > 0) {

	if (!$category_id == '') {
		$ids = getSubCatList($category_id);
		$aviable = array();
		$aviable = explode(',', $ids);
		$catserch = "AND category regexp '[[:<:]](" . implode('|', $aviable) . ")[[:>:]]'";
	}
	//echo $catserch;
	$sqls = "SELECT p.id, p.date, p.short_story, p.xfields, p.title, p.category, p.alt_name FROM " . PREFIX . "_post p LEFT JOIN " . PREFIX . "_post_extras e ON (p.id=e.news_id) WHERE p.approve=1 AND p.arhiv=0 AND p.date >= '$this_month' - INTERVAL 12 MONTH AND p.date < '$this_month' $catserch ORDER BY rating DESC, comm_num DESC, news_read DESC, date DESC LIMIT 0,{$config['top_number']}";
