...
if ($prosmotrbol){
$tex[] = "<tr onclick='window.open(\"{$link}&stroka=ok&cluck={$result2['id']}\",\"_self\");' class='trhis titleModal light {$cls}' title='{$result2['title']}' valign='center' data-placement='left'><td class='tdhis'>{$approv}</td><td class='tdhis'>{$dat}</td><td class='tdhis'>{$result2['autor']}</td></tr></table></div>";	
}else {
$tex[] = "<tr onclick='window.open(\"{$link}&cluck={$result2['id']}\",\"{$sel}\");' class='trhis titleModal light {$cls}' title='{$result2['title']}' valign='center' data-placement='left'><td class='tdhis'>{$approv}</td><td class='tdhis'>{$dat}</td><td class='tdhis'>{$result2['autor']}</td></tr></table></div>";
}
...
if ($prosmotrbol){
	$tex[] = "<tr onclick='window.open(\"{$link}&stroka=ok&cluck={$rows3['ids']}\",\"_self\");' class='trhis titleModal light {$cls}' title='{$rows3['title']}' valign='center' data-placement='left'><td class='tdhis'>{$approv}</td><td class='tdhis'>{$dat}</td><td class='tdhis'>{$rows3['autor']}</td></tr>";
}else {
  $tex[] = "<tr onclick='window.open(\"{$link}&cluck={$rows3['ids']}\",\"{$sel}\");' class='trhis titleModal light {$cls}' title='{$rows3['title']}' valign='center' data-placement='left'><td class='tdhis'>{$approv}</td><td class='tdhis'>{$dat}</td><td class='tdhis'>{$rows3['autor']}</td></tr>";
}
}
if ($est)
$tex[] = "</table></div>";
