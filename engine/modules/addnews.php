...
else $canonical = $PHP_SELF . "?do=addnews";
...
if (!$allow_addnews) {
  ...
  if (
      ...
      if ($found) {
        ...
        if ($_POST['autor12'] != "") {
          ...
          if (count($temp_array)) {
            $db2 = new db;
            foreach ($temp_array as $value) {
              $value = $db2->super_query("SELECT name FROM dle_users WHERE fullname = '{$value}'")['name'];
              if (trim($value)) $tags_array[] = trim($value);
            }
            $db2->close();
            $db2->free();
          ...
          if (count($tags_array)) $gtaut = implode(", ", $tags_array);
          else $gtaut = "";
        ...
        $db->query("UPDATE " . PREFIX . "_post set $atr title='$title', short_story='$short_story', full_story='$full_story', xfields='$filecontents', category='$category_list', alt_name='$alt_name', allow_comm='$allow_comm', approve='$approve', allow_main='$allow_main', fixed='$news_fixed', allow_br='$allow_br', tags='" . $_POST['tags'] . "' WHERE id='{$id}'");
        ...
        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        // Отправка на почту если опубликовано
        /*  if ($approve==1){
			$ids = $row[ 'id' ] ;
        ...
        if (
          ...
          foreach ($_POST['tags'] as $value) {
            ...
            $tags[] = "('" . $row['id'] . "', '" . trim($value) . "', '" . $category_list . "')";
          }
    ...
    if ($user_group[$member_id['user_group']]['moderation']) {
      if ($myautr == "") $myautr = $member_id['name'];
      $db2 = new db;
      $myautr = $db2->super_query("SELECT fullname FROM dle_users WHERE name = '$myautr'")['fullname'];
      $db2->close();
      $db2->free();
      $autr =  '<label for="alt_name"  class="imp">Автор</label>
...
function preview(){";
	...
	function checkxf() {
        ...
        if ($(".styledSelect").text()=='Выберите категорию...'){
...
HTML;
    ...
    $script .= "<form method=\"post\" name=\"entryform\" id=\"entryform\" onsubmit=\"if(!checkxf()){return false;}else {proverload('start');}\" action=\"\">";
