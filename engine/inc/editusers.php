...
if ($action == "list") {
	...
	if (intval($_REQUEST['news_per_page']) > 0) $news_per_page = intval($_REQUEST['news_per_page']);
	else $news_per_page = 50;
...
HTML;
	...
	if ($disabled_news) $ifch1 = "checked";
	else $ifch1 = "";
	if ($disabled_comments) $ifch2 = "checked";
	else $ifch2 = "";

	if ($_REQUEST['avtoriz_ad'] == "1") {
		$avtriz = "yes";
		$avfch = "checked";
	} else {
		$avtriz = "";
	  ...
	  <div class="col-md-5">
		...
		<div class="form-group">
			...
			<div class="col-sm-6">
				...
				<div class="checkbox"><label><input class="icheck" type="checkbox" name="avtoriz_ad" id="search_avtoriz" value="1" $avfch>{$lang['avtoriz_ad']}</label></div>
		  </div>
		</div>
...
HTML;
	...
	if (!empty($_REQUEST['avtoriz_ad'])) {
		$_REQUEST['avtoriz_ad'] = $db->safesql($_REQUEST['avtoriz_ad']);
		$where[] = "avtoriz_ad='1'";
	}
...
HTML;
	...
	if ($row['avtoriz_ad'] == 1) $avfch = "checked";
	...
	if ($row['news_subscribe']) $row['news_subscribe'] = "checked";
	else $row['news_subscribe'] = "";
	...
	if ($row['comments_reply_subscribe']) $row['comments_reply_subscribe'] = "checked";
	else $row['comments_reply_subscribe'] = "";
	...
	if (!$skin) {
		...
		if (count($ignore_list)) $ignore_list = implode("</span>, ", $ignore_list) . "</span>";
		else $ignore_list = "";

		if ($config['twofactor_auth']) {

			if ($row['twofactor_auth']) $row['twofactor_auth'] = "checked";
			else $row['twofactor_auth'] = "";
...
HTML;
		...
		echo <<<HTML
				<div class="form-group">
				  <label class="control-label col-md-3 col-sm-3">{$lang['avtoriz_ad']}</label>
				  <div class="col-md-9 col-sm-9">
				  	<input class="icheck" type="checkbox" name="avtorizad" value="yes" $avfch>
				  </div>
				 </div>
				<div class="list-group-divider"></div>
HTML;
...
HTML;
	...
	if ($user_group[$member_id['user_group']]['admin_editusers']) {
		...
		if ($_POST['banned'] and $row['user_group'] != 1) $banned = "yes";
		else $banned = "";
