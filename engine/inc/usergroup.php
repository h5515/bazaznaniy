...
if (isset($_REQUEST['id']) && intval($_REQUEST['id'] <  $member_id['user_group'])) {
    msg("error", $lang['addnews_denied'], $lang['db_denied']);
}
...
if ($action == "del") {
    ...
    if (!$row['count']) {
        ...
        @unlink(ENGINE_DIR . '/cache/system/'.$catl.'usergroup.php');
        clear_cache();
    ...
    if (!is_array($_REQUEST['allow_bz'])) $_REQUEST['allow_bz'] = array();
    ...
    if (!count($_REQUEST['allow_bz'])) $_REQUEST['allow_bz'][] = "all";
    ...
    if (empty($_COOKIE['dbname'])) {
        $list = array();

        $bd2 = new db;
        $arbz = implode("','", $_REQUEST['allow_bz']);
        $sqls = "SELECT id FROM dle_project WHERE Name in('{$arbz}')";
        $row = $bd2->query($sqls);
        foreach ($row as $value) {
            if (intval($value['id']) > 0) $list[] = intval($value['id']);
        }
        $bd2->free();

        $allow_bz = $db->safesql(implode(',', $list));

        if ($allow_bz == '') {
            foreach ($_REQUEST['allow_bz'] as $value) {
    ...
    //superadmin
    if ($user_group[$id]['allow_bz']) {
        $allow = $user_group[$id]['allow_bz'];
        $an = explode(',', $allow);
    }

    if (empty($_COOKIE['dbname'])) {
        if ($user_group[$id]['allow_bz'] == "all") $allow_bz_value = "selected";
        $row = $db->query("SELECT Name, id FROM " . USERPREFIX . "_project");
        while ($row = $db->get_row()) {
            ...
            $bz_list .= "<option " . $sel . ">" . $row['Name'] . "</option>";
        }
    }
    ...
    if (empty($_COOKIE['dbname'])) {
        $allow_bz = '
    <tr>
        <td class="white-line" style="width:58%"><h6 class="media-heading text-semibold">Доступ к БЗ</h6><span class="text-muted text-size-small hidden-xs">Предоствить доступ к базам знаний.</span></td>
        <td class="white-line" style="width:42%"><select data-placeholder="Выберите базу знаний." name="allow_bz[]" style="width:100%; max-width:350px;" class="cat_select" multiple >
<option value="all" '.$allow_bz_value.'>'.$lang["edit_all"].'</option>
'.$bz_list.'
</select></td>
    </tr>';
    }
