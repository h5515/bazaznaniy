...
if ( $config['allow_cache'] AND !$config['cache_type'] ) {
	...
	$stats_cache = @file_get_contents( ENGINE_DIR . "/cache/".$catl."news_adminstats.tmp" );
...
if ( !count($stats_arr) ) {
	...
	if ( $config['allow_cache'] AND !$config['cache_type'] ) {
		file_put_contents (ENGINE_DIR . "/cache/".$catl."news_adminstats.tmp", json_encode( $stats_arr, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES ), LOCK_EX);
		@chmod( ENGINE_DIR . "/cache/".$catl."news_adminstats.tmp", 0666 );
	}
