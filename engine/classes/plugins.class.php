...
abstract class DLEPlugins {
	...
	public static function Check($source) {
		...
		if( isset(self::$plugins[$check_file]) ) {
			...
			if( file_exists( ENGINE_DIR.'/cache/system/plugins/'.$catl.self::$plugins[$check_file] ) ) {
				
				return ENGINE_DIR.'/cache/system/plugins/'.$catl.self::$plugins[$check_file];
	...
	private static function loadplugins() {
		...
		if( !is_dir( ENGINE_DIR . "/cache/system/plugins".$catalog  ) ) {
				
			@mkdir( ENGINE_DIR . "/cache/system/plugins".$catalog , 0777 );
			@chmod( ENGINE_DIR . "/cache/system/plugins".$catalog , 0777 );
	
		}
	
		if( !is_dir( ENGINE_DIR . "/cache/system/plugins".$catalog ) OR !is_writable( ENGINE_DIR . "/cache/system/plugins".$catalog  )) {
		...
		if( count($files) ) {
			...
			if (isset($_COOKIE['dbname'])) {
				...
				$catl = $_COOKIE['dbname'].'/';
						...
						@file_put_contents (ENGINE_DIR . "/cache/system/plugins/".$catl . $store_key, $content, LOCK_EX);
						@chmod( ENGINE_DIR . "/cache/system/plugins/" . $store_key, 0666 );
						@chmod( ENGINE_DIR . "/cache/system/plugins/".$catl . $store_key, 0666 );
	...
	private static function getcache() {
		...
		if( file_exists(  ENGINE_DIR . '/cache/system/'.$catl.'plugins.php' ) ) {
			
			$data = file_get_contents( ENGINE_DIR . '/cache/system/'.$catl.'plugins.php' );
	...
	private static function setcache( $data ) {
		...
		if (isset($_COOKIE['dbname'])) {
			...
			@file_put_contents(ENGINE_DIR . '/cache/system/'.$catl.'plugins.php', json_encode( $data, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES ), LOCK_EX);
			@chmod( ENGINE_DIR . '/cache/system/'.$catl.'plugins.php', 0666 );
