...
class dle_template
{
  ...
  private function get_idcategories($id)
  {
  ...
  function load_template($tpl_name)
  {
    ...
    if (strpos($this->template, "{link-project") !== false) {
      if (isset($_COOKIE['dbname']))
        $prjlink = '&project=' . $_COOKIE['dbname'];
      else
        $prjlink = '';

      $this->template = str_ireplace("{link-project}", $prjlink, $this->template);
    }
    ...
    if (stripos($this->template, "category-") !== false) {
      ...
      if ($cat_id) {
        ...
        if (
          !$this->is_custom and $tpl_name != "shortstory.tpl"
          and $tpl_name != "fullstory.tpl"
        ) {
          if (isset($_COOKIE['dbname']))
          $prjlink = '&project=' . $_COOKIE['dbname'].'&';
          else
          ...
          else $this->template = str_ireplace("{category-url}", "$PHP_SELF?do=cat&{$prjlink}category={$cat_info[$cat_id]['alt_name']}", $this->template);
    ...
    if (strpos($this->template, "{subcatgl") !== false) {

      $cat = $this->get_idcategories(1);
      $cit = $this->getSubCatList($cat);

      $this->template = str_ireplace("{subcatgl}", $cit, $this->template);
    }
    if (strpos($this->template, "{subcat") !== false) {
      $cat = intval($category_id);

      $cit = $this->getSubCatList($cat);

      $this->template = str_ireplace("{subcat}", $cit, $this->template);
    }
  ...
  function sub_load_template($tpl_name)
  {
    ...
    if (strpos($template, "{link-project") !== false) {
      if (isset($_COOKIE['dbname']))
        $prjlink = '&project=' . $_COOKIE['dbname'];
      else
        $prjlink = '';

      $template = str_ireplace("{link-project}", $prjlink, $template);
    }
    ...
    if (strpos($template, "category-") !== false) {
      ...
      if ($cat_id) {
        ...
        else $template = str_ireplace("{category-url}", "$PHP_SELF?do=cat&{$prjlink}category={$cat_info[$cat_id]['alt_name']}", $template);
  ...
  function check_group($matches)
  {
    ...
    if (is_array($matches)) {
      ...
      if ($matches[1] == "group") $action = true;
      else $action = false;
      ...
      if ($action) {
        ...
        if (!in_array($member_id['user_group'], $groups)) $matches = '';
        else $matches = $block;
      } else {
  ...
  function check_device($matches = array())
  {
    ...
    if (
      $matches[1] == "smartphone"
      or $matches[1] == "tablet"
      or $matches[1] == "desktop"
    ) $action = true;
    else $action = false;
    if (
      $matches[1] == "smartphone"
      or $matches[1] == "not-smartphone"
    ) $device = $this->smartphone;
    if (
      $matches[1] == "tablet"
      or $matches[1] == "not-tablet"
    ) $device = $this->tablet;
  ...
  function check_page($matches = array())
  {
    ...
    if ($matches[1] == "page-count") $action = true;
    else $action = false;
  ...
  function check_tag($matches = array())
  {
    ...
    if (
      $matches[1] == "tags"
      or $matches[1] == "not-tags"
    ) $tag = "tags";
    if (
      $matches[1] == "news"
      or $matches[1] == "not-news"
    ) $tag = "news";
  ...
  function build_cat_menu($matches = array())
  {
    ...
    if ($config['category_newscount']) $cache_prefix = "news";
    else $cache_prefix = "catmenu";
  ...
  function compile_item($row, $template)
  {
    ...
    if ($config['allow_alt_url']) {
      ...
      $template = str_replace("{url}", $config['http_home_url'] . "index.php?do=cat&amp;{$prjlink}category=" . $row['alt_name'], $template);
    }
