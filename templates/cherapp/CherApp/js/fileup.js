...
(function($) {
    ...
    function uploadFile(input, fileContainer) {
        ...
        vil = translit(file.name);

        if (window.FormData !== undefined) {
            var formData = new FormData();
            formData.append(options.fieldName, file, vil);
            ...
            $.ajax({
                ...
                success: function(respond, textStatus, jqXHR) {
                    events.callEvent(input, 'progress', [file_number, 700, file]);
                    events.callEvent(input, 'success', [xhr.responseText, file_number, file]);
                    // Если все ОК
                    ...
                    $.post(hv_root + "engine/ajax/controller.php?mod=cherapp", {
                        ...
                        $('#bottom_modal [data-dismiss="modal"]').on('click', function() {
                            im = $(this).attr('src');
                            dataid = $('#bottom_modal .modal-body').attr('data-id');
                            $('#' + dataid).attr('src', im);
                            ProverTempBol();
                        });
                    }).fail(function(b) {
                    ...
                    setTimeout(() => {
                        $.fileup(options.inputID, 'remove', file_number);
                    }, 4000);
                },
